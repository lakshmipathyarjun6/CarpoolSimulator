...
(function (jss) {
    ...
    Space2D.prototype.updateAgentRider = function (rider, x, y, endx, endy) {
        this.locations[rider.id] = new jss.Vector2D(x, y);
        this.endlocations[rider.id] = new jss.Vector2D(endx, endy);
        this.agents[rider.id] = rider;
    };
    ...
    Space2D.prototype.render = function (canvas) {
        ...
        for(id in this.locations){
            ...
            var maybeEndLocation = this.endlocations[id];
            
            if(agent.draw) {
                agent.draw(context, new jss.Vector2D(pos.x, canvas.height - pos.y));
                if(maybeEndLocation) {
                    agent.draw(context, new jss.Vector2D(maybeEndLocation.x, canvas.height - maybeEndLocation.y));
                }
            } else {
                ...
                if(agent.id){
                    ...
                    context.fillText("" + agent.id,pos.x-0.5*width, canvas.height - pos.y - 0.5*height);
                    if(maybeEndLocation) {
                        context.fillText("" + agent.id + 'e',maybeEndLocation.x-0.5*width, canvas.height - maybeEndLocation.y - 0.5*height);
                    }
                } 
                context.fillRect(pos.x-0.5*width, canvas.height - pos.y - 0.5*height, width,height);
                if(maybeEndLocation) {
                    context.fillRect(maybeEndLocation.x-0.5*width, canvas.height - maybeEndLocation.y - 0.5*height, width,height);
                }
            }
        }  
    };
